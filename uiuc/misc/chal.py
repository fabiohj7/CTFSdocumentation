...
class CoolDownTea(ast.NodeTransformer):
    def visit_Call(self, node: ast.Call) -> ast.AST:
        return ast.Call(
            func=ast.Name(id="safe_call", ctx=ast.Load()), args=[], keywords=[]
        )

    def visit_Import(self, node: ast.AST) -> ast.AST:
        return ast.Expr(
            value=ast.Call(
                func=ast.Name(id="safe_import", ctx=ast.Load()), args=[], keywords=[]
            )
        )
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> ast.AST:
        return ast.Expr(
            value=ast.Call(
                func=ast.Name(id="safe_import", ctx=ast.Load()), args=[], kewords=[]
            )
        )
...
astpretty.pprint(cup)

exec(
    compile(cup, "", "exec"),
    {"__builtins__": {}},
    {"safe_import": safe_import, "safe_call": safe_call},
)
